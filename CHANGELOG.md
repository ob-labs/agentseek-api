...
- Reduced inline execution overhead by batching live stream-event writes and
  final thread-history checks. Pending events are drained before terminal
  publication, including execution failure and cancellation.
- Preserved ordered replay when buffered events and saved events overlap,
  with saved payloads taking precedence over mutable in-memory copies.
- Hardened `dev` readiness checks on macOS with explicit IPv4/IPv6 loopback
  targets, proxy bypass for local probes, and clearer timeout reporting.

### Validation

- Added native embedded seekdb performance and exact-replay benchmarks, with
  timing and SQL-operation budgets in CI. The
  [performance report](docs/performance/embedded-seekdb-performance.md)
  records the methodology and macOS/Linux measurements.
- Synchronized package metadata, container runtime pins, manifests, labels,
  scripts, and fixtures at 0.3.2.

### Upgrade notes

- No API or database schema migration is required. Inline stream persistence
  remains best effort on database failures; an abrupt process failure can
  lose events that have not yet been flushed. Completed-run checkpoints and
  the Redis persistence path retain their existing behavior.
- Container projects whose dependency constraints exclude 0.3.2 must update
  their runtime pin before using containers generated by this release.
