...
async def claim_due_crons(
    *,
    ...
    queue = RedisRunQueue() if settings.EXECUTOR_BACKEND.strip().lower() == "redis" else None
    session_factory = db_manager.get_session_factory()
    try:
        async with session_factory() as session:
            ...
            for tick in reclaimed_ticks:
                ...
                tick.updated_at = current_time
                claimed.append(
                    ClaimedCron.from_row(
                        row,
                        tick_id=tick.id,
                        scheduled_for=tick.scheduled_for,
                        thread_id=tick.thread_id,
                        run_id=tick.run_id,
                    )
                )
                seen_cron_ids.add(row.cron_id)

            reclaimed_queued_ticks = list(
                (
                    await session.scalars(
                        select(CronTick)
                        .where(
                            CronTick.status == "queued",
                            CronTick.run_id.is_not(None),
                            CronTick.updated_at <= stale_before,
                        )
                        .order_by(CronTick.scheduled_for.asc(), CronTick.id.asc())
            ...
            for tick in reclaimed_queued_ticks:
                row = await session.scalar(select(CronJob).where(CronJob.cron_id == tick.cron_id))
                if row is None:
                    continue
                run = await session.scalar(select(Run).where(Run.run_id == tick.run_id))
                if run is None or run.status != "pending":
                    continue
                if queue is not None and await queue.contains_run(run_id=run.run_id):
                    continue
                tick.scheduler_id = scheduler_id
                tick.updated_at = current_time
                claimed.append(
...
async def dispatch_claimed_cron(claim: ClaimedCron) -> CronDispatchResult:
    ...
    queued_run_id = claim.run_id
    result_thread_id = claim.thread_id
    try:
        if claim.run_id is not None:
            run = await submit_existing_run(run_id=claim.run_id)
        ...
        if claim.thread_id is None:
            ...
            result_thread_id = thread.thread_id
            ...
            run = await submit_existing_run(run_id=run.run_id)
            return CronDispatchResult(
                cron_id=claim.cron_id,
                status="queued",
                thread_id=thread.thread_id,
