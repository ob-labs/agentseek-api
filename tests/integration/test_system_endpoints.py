...
def test_info_endpoint_keeps_a2a_false_without_runtime_mount_when_config_allows(
...
""".strip(),
    ...
    assert response.status_code == 200
    assert response.json()["flags"]["a2a"] is False
    assert app.state.a2a_config_enabled is True
    assert app.state.a2a_enabled is False
    auth_middleware._backend = None
    langgraph_service_module._langgraph_service = None


def test_info_endpoint_keeps_a2a_false_without_runtime_mount_when_config_disables(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setattr("agentseek_api.core.database.OceanBaseCheckpointSaver", FakeCheckpointer)
    monkeypatch.setattr(settings, "SEEKDB_URL", f"sqlite+aiosqlite:///{tmp_path}/test.db")
...
""".strip(),
    ...
    assert response.status_code == 200
    assert response.json()["flags"]["a2a"] is False
    assert app.state.a2a_config_enabled is False
    assert app.state.a2a_enabled is False
    auth_middleware._backend = None
    langgraph_service_module._langgraph_service = None
